...
# 1. INITIALIZE WEB INTERFACE LAYER
st.set_page_config(page_title="UrbanAI Nexus | Deep Generative Engine", layout="wide")

# Inject a high-end, clean dark UI design profile
st.markdown("""
    <style>
...
st.title("⚡ UrbanAI Nexus™ — Deep Generative Smart City Engine")
st.markdown("---")
...
if uploaded_file is not None:
    ...
    # Transform raw image pixels into AI readable tensors
    ...
    with st.spinner("⚡ Running Deep Neural Urban Synthesis..."):
        with torch.no_grad():
            generated_tensor = net_G(input_tensor)
        
        # De-normalize tensor values back into professional standard RGB images
        output_display = (generated_tensor.squeeze(0).cpu() + 1.0) / 2.0
        output_np = (output_display.permute(1, 2, 0).numpy() * 255).astype(np.uint8)
        
        # Resize output map for clear presentation display
        final_blueprint = Image.fromarray(output_np).resize((600, 600), Image.Resampling.NEAREST)
        input_display_img = raw_img.resize((600, 600), Image.Resampling.LANCZOS)

    # Render clean side-by-side presentation columns
    ...
    with layout_col2:
        st.subheader("🗺️ AI Synthesized Neural Blueprint Layout")
        st.image(final_blueprint, use_container_width=True)
        
    # Enable crisp file export
    ...
    st.info("ℹ️ System standing by. Upload high-resolution aerial terrain imagery to initiate the neural pipeline.")
